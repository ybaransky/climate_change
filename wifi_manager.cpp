...
void WiFiManager::setupConfigPortal() {
  ...
  if (_apHandlersCallback)
    _apHandlersCallback(this);
  else
  {
    server->on("/",         std::bind(&WiFiManager::handleRoot, this));
    server->on("/home",     std::bind(&WiFiManager::handleRoot, this));
...
boolean WiFiManager::autoConnect() {
  String ssid = "YURIJ-" + String(ESP.getChipId());
...
#ifdef YURIJ_ADDITION  
  WiFi.hostname(_apName);  // 20171001 yurij make the hostame of this client the same as the AP name
#endif
...
void WiFiManager::handleRoot(void) {
  ...
  {
    for (WiFiManagerButton* btn : _buttons)
    {
      String line = FPSTR(HTTP_PORTAL_OPTION);
      line.replace("{x}", btn->path());
      line.replace("{y}", btn->name());
      page += line;
    }
  }
  page += FPSTR(HTTP_END);
...
void WiFiManager::handleWifiSave() {
  ...
  Serial.printf("yurij: ssid='%s'  password='%s'\n", _ssid.c_str(), _pass.c_str());
  //parameters
  for (int i = 0; i < _paramsCount; i++) {
    if (_params[i] == NULL) {
      break;
...
void WiFiManager::handleInfo() {
  ...
  page += F("</dd>");
#ifdef YURIJ_ADDITION
  page += F("<dt>Temperature Sensor #1</dt><dd>");  // 20171104 yurij
  page += F("<a href='https://www.bosch-sensortec.com/bst/products/all_products/bme280'>Bosch BME280</a>");
  page += F("</dd>");
  page += F("<dt>Temperature Sensor #2</dt><dd>");  // 20171104 yurij
  page += F("<a href='https://www.sensirion.com/en/environmental-sensors/humidity-sensors/digital-humidity-sensors-for-various-applications/'>Sensirion SHT31</a>");
#endif
  page += F("</dd>");
  page += F("</dl>");
  page += FPSTR(HTTP_END);
...
#ifdef YURIJ_ADDITION
  WiFi.disconnect();  // should erase credentials
  yield();
  delay(3000);
  ESP.reset();
  delay(1000);
